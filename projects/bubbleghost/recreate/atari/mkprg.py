...
COPIED FROM projects/zynaps/recreate/atari/mkprg.py (itself a copy of Joust's), and identical to
both BELOW THIS PARAGRAPH — verified at copy time by diffing past the headers; each copy names the
others, so the markers differ and nothing else does. A change to any of them belongs in all.
...
NO LONGER IDENTICAL TO THE ZYNAPS AND JOUST COPIES, and here is the whole of the difference: `NM`
and `nm_rows` below, with `sym_value` rewritten to take one row out of it. `nm_rows` is the ONE
parse of `nm`'s output, and `atari/profile.py` builds a whole Hatari symbol table out of it — which
is why it landed here first. It is behaviour-neutral in the other copies (`sym_value` is their only
caller), and projects/wonderboy/recreate/atari/mkprg.py already carries the same addition and says
the same thing, so BOTH belong in zynaps' and joust's copies whenever someone is in them next.
"""
import re
import struct
import subprocess
