...
Thin wrapper over run_golden.py's ONE golden harness in its ste=True mode: it builds the STE variant of
each leg's GOLDEN.PRG (GAME_STE=1 + -DGOLDEN_BOOT_LEG=N) and boots it on `hatari --machine ste --blitter`,
byte-comparing the boot frame against recreate's pipeline golden. A MATCH on all legs proves the STE
build's whole render pipeline draws the SAME pixels as the stock ST build (the object blits still run the
