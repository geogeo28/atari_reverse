#!/usr/bin/env python3
"""run_demo.py — run the interactive road + HUD .PRG on a headless Hatari and verify its first frame.
...
g_render_road + g_draw_hud for the same pose). A MATCH proves remaster's full road + HUD pipeline —
the geometry builder, the road rasterizer and the HUD — cross-compiled and executed on a real 68000
core, produces the exact same pixels as the verified recreate cores. A PNG is also written.
...
def main():
    ...
    if bytes(fb) == golden:
        print("MATCH: on-target road + HUD is byte-identical to recreate's ported pipeline")
    else:
