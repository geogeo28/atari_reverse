...
def main():
    ...
    out += [
        ...
        "",
        # ---- course-event engine: the EventState scalars the demo starts a leg with (recreate's
        # globals; the leg-0 dashboard rebuild + the checkpoint/collision markers advance them).
        f"#define EV_COURSE_FLAG_BIT_INIT {ev.course_flag_bit}",
        # The dashboard-marker scalars {y, bit, x} are ARENA-derived: init_leg seeds them for the leg
        # from buf_a's per-leg dashboard-marker table. They are baked here (from the oracle's leg-start
        # image) rather than re-derived at demo boot from the loaded arena — deriving them would mean
        # porting that slice of init_leg. Drift risk: if the loaded arena's marker table for DEMO_LEG
        # ever changes, regenerate this fixture (as with every other baked leg-start scalar here).
        f"#define EV_DASH_Y_INIT        {ev.dash_y}",
        f"#define EV_DASH_BIT_INIT      {ev.dash_bit}",
        f"#define EV_DASH_X_INIT        {ev.dash_x}",
        f"#define EV_CRASH_BARS_INIT    {ev.crash_bars}",
        f"#define EV_CRASH_ACTIVE_INIT  {ev.crash_active}",
        f"#define EV_CRASH_LAP_INIT     {ev.crash_lap}",
        f"#define EV_GAUGE_BLINK_INIT   {ev.gauge_blink}",
        f"#define EV_GAUGE_BLINK_ON_INIT {ev.gauge_blink_on}",
        f"#define EV_CKPT_SCROLL_INIT   {ev.ckpt_scroll}",
        f"#define EV_SPIN_STATE_INIT    {ev.spin_state}",
