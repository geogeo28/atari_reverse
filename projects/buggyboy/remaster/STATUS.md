...
| fine-x blit engines (`blit_objshift`, `blit_objshift2`, objsprite) | `g_blit_objshift` / `_w2` / `g_blit_objshift2` / `g_objsprite_t*` | ✅ ported (C reference; `blit_objshift2` also has a shipped hand-asm core, PERF30 A3) | `test/test_blit_engines.py` — byte-exact fuzz across every fine-x, dispatch case (clip/edge/base/wide), colours, strides, all width families; `test/test_asm_blit.py` — Musashi C-vs-asm differential for the `blit_objshift2` hand-asm core (`src/asm/objshift2.S`) |
...
Current (8 MHz ST, 160000-cycle 50 Hz frame budget) — the game frame is **158.1 ms ≈ 6.3 fps** on the
staged frame (203 ms before PERF30 A1 landed on `rm_blit_objshift`; 163.8 ms before A3 dropped the
fixed-pass engine to hand-asm). Caveat before reading the object rows: the staged frame is the game's BOOT frame — a
...
| **TOTAL (frame)** | **157.6** | | | was 203.2 before A1, 194.6 after A1, 188.0 after P1/P2, 185.6 after P4, 179.6 after the review-fix fold, 172.1 after L1, 167.8 after the GCC-level sweep, 163.8 after C1+C3; **A3 phase 1 (objshift2 hand-asm) + its review fixes (F3 register-held rewinds) took it to 157.6** (funcs-sum basis; 1,310,066 → 1,260,780 cyc, −49,286 = −6.16 ms); recon (recreate-parity) is ~240 ms on this frame; the ORIGINAL asm is **110 ms (9.1 fps)** — remaster is still slower than the original here, NOT faster (the ~240 ms is the recon, not the original — see PERF30.md Part 0) |

Whole-tree check: `object_tree` (prefix→buggy, recreate's `g_draw_game_objects` scope) is 71.7 ms
