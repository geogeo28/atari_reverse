...
the demo (`demo_fixture.h`: 28530 → ~2613 lines, after `rm_init_leg` dropped the baked leg-start
...
- **A leg now ENDS (slice 1, host-side).** The leg/game-flow CORE is ported: `g_game_update` §1's
  ...
  **Slice 2 wired that on-target**: `demo_main.c`'s frame loop runs `rm_crash_fx_update` every frame at
  ...
  (and the boot) now run NATIVELY through `rm_init_leg`** (`demo_main.c` `start_leg`), which resets every
...
- **Known structural gap — the flow COMPOSITION has no host coverage (slice C).** The between-legs
  *pieces* are each differentially pinned host-side (`test_flow.py` / `test_flow_machine.py`), but the
  driver that SEQUENCES them — the prologue, the Phase A/B/C/D loop, the draws/flips/palettes, the
  leg-select loop, the quit unwind — lives in `demo_main.c` (`intermission_cycle` / `run_leg_select` /
  `main`), which `make test` never compiles or runs. Its only guard is the on-target flow-trace
  (`DEMO_FLOW_AUTO` phase log) plus the frame-0 golden `MATCH` — an integration smoke test, not a
  differential. That is why the four correctness bugs a review found here (leg-start marker not
  re-seeded, the idle trace hanging past the leg end, Esc/Q unreachable from menus, the menu-started
  race keeping the leg-select palette) could ship green. **Planned next slice:** hoist the composition
  driver out of the shell into `src/flow.c` behind draw / flip / palette callbacks, so the host harness
  can lockstep the whole A→B→C→D→leg-select sequence against recreate's `g_intermission` /
  `g_init_playfield` the way the individual phase steps already are — leaving `demo_main.c` only the
  68000 I/O implementations of those callbacks.
