...
- **The flow COMPOSITION now has host coverage (slice D — closes the slice-C gap).** The driver that
  ...
  leg-select loop, the game-over enter/exit bracket, the quit unwind — is hoisted out of `demo_main.c`
  ...
  knobs, the DEMO_FLOW_FAST debug seeds promoted to data). `demo_main.c` keeps ONLY the 68000
  ...
  ONLY by the on-target `DEMO_FLOW_AUTO` flow trace + the frame-0 golden `MATCH`, which also still guard
