...
Current (8 MHz ST, 160000-cycle 50 Hz frame budget) — the game frame is **167.8 ms ≈ 6.0 fps** on the
...
| **objlist pass 1 (sprites)** | **28.74** | — | | ~79% inside `rm_blit_objshift`; **PERF30 A1 + P1/P2 + P4a + review-fix fold + GCC-level sweep landed** (…P4a → 34.47; review-fix folded the disguised-constant row step → 31.70; **GCC-level sweep: global -O3 + a per-function `optimize("-fira-region=one","-fira-algorithm=priority")` on `rm_blit_objshift` let IRA place the pressure-bound cursor triple — the function itself 199,152 → 181,836 cyc (−17,316), and the whole pass bench_objlist_pass1 253,564 → 231,480 (−22,084 = 31.70 → 28.93 ms; the extra −4,768 is -O3 on the dispatcher glue)**); P3 no-op reverted; **L2 swap-packing +14,352, reverted; E2 local register vars +15,208, dropped; E3 LTO breaks the harness, dropped**; **C3 (P4 pointer cursors on the objsprite family) 231,480 → 229,948 (rm_objsprite 18,204 → 16,672, −1,532 = 28.93 → 28.74 ms) and dropped the dead offset-wrapper helpers; C3's dispatcher-glue pass found only inherent ABI arg-marshalling (no surgical win)** — PERF30 "GCC-level sweep" + "C1/C2/C3") |
| draw_object | 0.90 | — | | |
| objlist pass 2 | 0.05 | — | | empty on this frame |
| **objlist fixed pass** | **40.68** | — | | 97% inside `rm_blit_objshift2`; A1 does NOT apply (value-passing REGRESSED it +1.98 ms, PERF30 A1) — stays by-pointer; **P4b + review-fix fold + L1 straddle-unroll + -O3 sweep + C1 landed** (…L1 → 345,264 cyc; global -O3 shaved a further −834 to `rm_blit_objshift2` 356,436; region=one/priority REGRESS objshift2 (+4,440) so its attribute is objshift-only; **C1 specialized the straddle-cell run (0..3, loop-invariant) as an always_inline fall-through switch — no per-cell addql/cmpl/bne — `rm_blit_objshift2` 345,264 → 314,216 cyc (−31,048 = 43.16 → 39.28 ms), bench_objlist_fixed 356,436 → 325,424 (44.55 → 40.68 ms); C2 mask-build reshaping tried twice, both REGRESSED (+3,888 / +10,966), reverted** — PERF30 "GCC-level sweep" + "C1/C2/C3") |
...
| **TOTAL (frame)** | **163.8** | | | was 203.2 before A1, 194.6 after A1, 188.0 after P1/P2, 185.6 after P4, 179.6 after the review-fix fold, 172.1 after L1, 167.8 after the GCC-level sweep; **C1 (objshift2 straddle switch) + C3 (objsprite pointer cursors) took it to 163.8** (funcs-sum basis; 1,342,610 → 1,310,066 cyc, −32,544 = −4.07 ms); recon (recreate-parity) is ~240 ms on this frame; the ORIGINAL asm is **110 ms (9.1 fps)** — remaster is still slower than the original here, NOT faster (the ~240 ms is the recon, not the original — see PERF30.md Part 0) |

Whole-tree check: `object_tree` (prefix→buggy, recreate's `g_draw_game_objects` scope) is 77.9 ms
(was 117.2 before A1, 108.6 after A1, 102.0 after P1/P2, 99.6 after P4, 93.6 after the review-fix fold, 86.1 after L1, 82.0 after the GCC sweep, 77.9 after C1+C3)
vs the recon's 130.3 ms (**0.60×**). `render_road` also beats the byte-exact **machine model**
