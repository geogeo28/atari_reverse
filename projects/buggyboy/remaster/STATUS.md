...
| draw_hud | 17.64 | 17.20 | 1.03× | 10.6 in the phases (dashboard masked blit), 6.0 in glyph_run; +0.17 under -O3 (within noise, outweighed by the objsprite wins). **Measured on the BAKED atlas** — the staged bench frame carries the leg-independent, 100 %-opaque dashboard graphic, so `cell_dashboard`'s `mask == 0` fast path takes every group. On the LIVE per-leg mini-map `init_leg_dash` builds (transparent), the same blit costs **67,978 cyc / 8.50 ms** instead of 5.70 and real in-race draw_hud is **19.06 ms** (the fast path fires on 1–9 of 320 groups); anywhere this is compared against the original, say which art. **On an STE the dashboard composite is ROUTED to the hardware blitter** (8-pass AND/OR cookie-cut read IN PLACE out of the graphics arena — no table, no materialise; `src/blitter_dash.c`, BLIT_STE_SPEC §17) — this row is the 68000 reference both machines share. **Honestly unpinned on that route** (BLIT_STE_SPEC §17 "Stated limitations"): its odd-address tripwire is structurally unreachable (both buffers and the arena are word-aligned by construction, so no build can seed the decline), it does not cover the chip's 24-bit address truncation, and the BOOT BIND is evidenced only by the cadence `dash routed` counter — the sweep drives the route directly and the goldens compare pixels that are identical either way |
