...
def compare_hud(lib, image):
    ...
    footprint; wrong_bytes = count of bytes the candidate changed to a value recreate didn't. The
    candidate's phase-7 dashboard is an on-the-fly MASKED blit that composites over the live frame,
    exactly as recreate's g_draw_hud does (the per-leg dashboard is transparent — see src/hud.c)."""
...
class _ComposedScene:
    ...
    def __init__(self, lib, image, cand):
        ...
        # The phase-7 dashboard is drawn on-the-fly by rm_draw_frame's HUD as a MASKED blit that
        # composites over the live frame (the per-leg mini-map is transparent — see src/hud.c), exactly
        # as recreate's g_draw_hud does, so nothing is prebuilt here.
    ...
    def draw(self, state):
        ...
        cand = self.cand
        # (1) sync the two arenas the drive mutates into the image the assets point into. The dashboard
        # art (with the progress marker probe_collision moves on WRAP frames) is re-blitted masked from
        # buf_c every frame by rm_draw_frame's HUD, so no prebuild/invalidation is needed here.
