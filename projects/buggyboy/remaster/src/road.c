...
/* ---- blit primitives: dst is a cursor into fb->px, src a native texture cursor; both post-increment
 * like the 68k `(an)+` addressing. ---- */

/* copy one long (half a 16-pixel 4-plane column) src->dst, advancing both. */
static inline void rr_copy_long(Framebuffer *fb, Offset *dst, const uint8_t **src) {
    wr32(fb->px + *dst, be32(*src)); *src += 4; *dst += 4;
}
/* copy one long masked by the full-long edge mask. */
static inline void rr_copy_long_masked(Framebuffer *fb, Offset *dst, const uint8_t **src, Plane4 mask) {
    wr32(fb->px + *dst, be32(*src) & mask); *src += 4; *dst += 4;
}
/* write one forward shoulder-fill pair (interior plane long, then edge plane long). */
static inline void rr_fill_pair(Framebuffer *fb, Offset *dst, Plane4 lo, Plane4 hi) {
    wr32(fb->px + *dst, lo); *dst += 4; wr32(fb->px + *dst, hi); *dst += 4;
}
/* write one backward shoulder-fill pair (predecrement — the 68k `move.l dN,-(a0)` tail fill). */
static inline void rr_fill_pair_rev(Framebuffer *fb, Offset *dst, Plane4 lo, Plane4 hi) {
    *dst -= 4; wr32(fb->px + *dst, hi); *dst -= 4; wr32(fb->px + *dst, lo);
}
/* fill one full 160-byte scanline forward through the cursor (the 68k `moveq #9,dbf` ten-times loop
 * of (lo,hi,lo,hi) writes — i.e. RR_ROW_LONG_PAIRS (lo,hi) pairs). */
static inline void rr_fill_full_row(Framebuffer *fb, Offset *dst, Plane4 lo, Plane4 hi) {
    for (int i = 0; i < RR_ROW_LONG_PAIRS; i++) rr_fill_pair(fb, dst, lo, hi);
}
/* word RMW on the framebuffer: word[at] &= mask.w (the 68k `and.w dN,-4(a0)` masks a long's hi word). */
static inline void rr_andw(Framebuffer *fb, Offset at, Plane4 mask) {
    wr16(fb->px + at, (uint16_t)(be16(fb->px + at) & (uint16_t)mask));
}
/* solid-fill a whole 160-byte scanline starting at `at`. */
static void rr_fill_row(Framebuffer *fb, Offset at, Plane4 lo, Plane4 hi) {
    for (int i = 0; i < RR_ROW_LONG_PAIRS; i++, at += 8) { wr32(fb->px + at, lo); wr32(fb->px + at + 4, hi); }
}
/* solid-fill the left shoulder [row_start, row_start + col): col is column-aligned. */
static void rr_fill_shoulder(Framebuffer *fb, Offset row_start, int16_t col, Plane4 lo, Plane4 hi) {
    for (int cell = (int)((uint16_t)col >> 3); cell > 0; cell--, row_start += 8) {
        wr32(fb->px + row_start, lo); wr32(fb->px + row_start + 4, hi);
    }
}
/* draw the antialiased edge cell (one 16-pixel column) at dst: two longs, the first hi-word masked. */
static void rr_draw_edge_cell(Framebuffer *fb, Offset *dst, const uint8_t **src, Plane4 mask) {
    Offset edge = *dst;
    rr_copy_long(fb, dst, src);
    rr_andw(fb, edge, mask);
    rr_copy_long(fb, dst, src);
}
...
/* backward left-shoulder fill: from `edge` leftward, writing (lo,hi) cells. `width` counts down a
 * cell (8 bytes) at a time; a cell still past the row end (>= stride) is skipped; the walk stops
 * when the position goes negative or the run `count` is exhausted. */
static void rr_band_A_shoulder_fill(Framebuffer *fb, Offset edge, int16_t width, uint32_t count,
                                    int16_t stride, Plane4 lo, Plane4 hi) {
    Offset shoulder = edge;
    for (;;) {
        width = rr_wsub((uint16_t)width, 8);
        if (width < 0) break;
        if (rr_wsub((uint16_t)width, (uint16_t)stride) >= 0) shoulder -= 8;   /* cell past the row: skip */
        else rr_fill_pair_rev(fb, &shoulder, lo, hi);
...
static void rr_band_A_interior_split(Framebuffer *fb, Offset *dst, const uint8_t **src, int16_t pos,
    ...
    if (rr_wadd((uint16_t)pos, 8) >= 0) {             /* narrow gap: 2 longs */
        ...
        rr_copy_long(fb, dst, src);
        rr_copy_long(fb, dst, src);
        if (masked_edge) rr_copy_long_masked(fb, dst, src, edge_mask);
        else             rr_copy_long(fb, dst, src);
    }
    pos = rr_wadd((uint16_t)rr_wadd((uint16_t)pos, 8), 8);
    while (pos < 0) { rr_fill_pair(fb, dst, gap_lo, gap_hi); pos = rr_wadd((uint16_t)pos, 8); }
}
...
static void rr_band_A_interior_center(Framebuffer *fb, Offset *dst, const uint8_t **src, int16_t pos,
    ...
    for (;;) {
        pos = rr_wadd((uint16_t)pos, 8);
        if (pos >= 0) break;
        rr_fill_pair(fb, dst, gap_lo, gap_hi);
        if (!rr_dbf(&count)) break;
    }
}
...
static void rr_band_A(rr_regs *r) {
    ...
    for (;;) {
        ...
        if (col < 0) {
            ...
            if (is_split) {
                ...
                if (rr_wadd((uint16_t)col, 8) >= 0) {           /* copy an edge cell first */
                    count = 0x12; src += 8;
                    if (plane_hi) { rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src); }
                    else          { rr_copy_long(fb, &dst, &src);
                                    rr_copy_long_masked(fb, &dst, &src, edge_mask); }
                }
                do { rr_fill_pair(fb, &dst, fill_lo, fill_hi); } while (rr_dbf(&count));
            } else {                                            /* narrow centre */
                src += 8;
                int16_t col_cells = rr_wadd((uint16_t)col, 8);
                if (col_cells >= 0) { rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src); }
        ...
        Offset edge = row_dst + sx16((uint16_t)col);
        Offset dst = edge;
        int16_t pos = rr_wsub((uint16_t)col, (uint16_t)stride);
        if (pos < 0) {
            if (is_split) rr_band_A_interior_split(fb, &dst, &src, pos, !plane_hi, edge_mask,
                                                   fill_lo, fill_hi);
            else          rr_band_A_interior_center(fb, &dst, &src, pos, count_long & 0xffff,
                                                    fill_lo, fill_hi);
        }
        Plane4 sh_lo, sh_hi;
        rr_band_A_shoulder_pattern(ctrl, /*center=*/!is_split, &sh_lo, &sh_hi);
        rr_band_A_shoulder_fill(fb, edge, col, count_long >> 16, stride, sh_lo, sh_hi);
...
static void rr_band_B(rr_regs *r, uint32_t rows_m1, int second) {
    ...
    for (;;) {
        ...
        if (!(ctrl & RR_F_SPLIT_B)) {                          /* no-split: edge-relative strip */
            ...
            r->edge += 2; r->dst += stride;                    /* flagged blank scanline */
            ...
            if (ctrl & RR_F_WIDE) {                            /* wide/solid-centre strip */
                ...
                if (ctrl & RR_F_SRC_400) {
                    src += RR_SRC_0400;
                    if (!(ctrl & RR_F_SRC_100)) src = r->edge_const + RR_CONST_5B7A;
                } else {
                    fill_lo = rr_notw(fill_lo);
                    if (!(ctrl & RR_F_SRC_100)) src = r->edge_const + RR_CONST_5B9A;
                }
        ...
        if (!second) {
            ...
            if (e < 0) {
                rr_fill_full_row(fb, &r->dst, fill_lo, fill_hi);   /* road off-screen: fill row */
            } else {
                int16_t rem = rr_wsub((uint16_t)e, (uint16_t)stride);
                if (rem < 0) {
                    rr_copy_long(fb, &dst, &src);
                    rr_andw(fb, dst - 4, edge_mask);               /* mask high word of the long */
                    rr_copy_long(fb, &dst, &src);
                    rem = rr_wadd((uint16_t)rem, 8);
                    while (rem < 0) {                              /* forward shoulder fill */
                        rr_fill_pair(fb, &dst, fill_lo, fill_hi);
                        rem = rr_wadd((uint16_t)rem, 8);
                    }
                }
                r->dst += stride;
        ...
        if (col < 0) {                                          /* road off-screen: counted fill */
            ...
            if (rem >= 0) {                                     /* one masked edge cell first */
                src += 8;
                rr_copy_long(fb, &dst, &src);
                rr_andw(fb, dst - 4, edge_mask);
                rr_copy_long(fb, &dst, &src);
            }
            do { rr_fill_pair(fb, &dst, fill_lo, fill_hi); } while (rr_dbf(&cells));
            r->dst += stride;
            if (!rr_dbf(&remaining)) break;
            continue;
        }
...
static void rr_band_C_near(rr_regs *r, uint32_t rows_m1) {
    ...
    for (;;) {
        ...
        if (fast_split) {
            ...
            if (col >= 0) {
                ...
                if (ctrl & RR_F_PLANE_HI) {
                    ...
                    if (!full_row) { rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src); }
                } else {
                    src += sx16(be16(r->edge - 2));
                    full_row = rr_wsub((uint16_t)col, (uint16_t)stride) >= 0;
                    if (!full_row) { rr_copy_long(fb, &dst, &src);
                                     rr_copy_long_masked(fb, &dst, &src, edge_mask); }
                }
                if (full_row) {
                    rr_fill_full_row(fb, &r->dst, fill_lo, fill_hi);
                    if (!rr_dbf(&remaining)) break;
                    continue;
                }
                ...
                if (shoulder_cells >= 0) {
                    Offset sh = r->dst; uint32_t cnt = (uint16_t)shoulder_cells;
                    do { rr_fill_pair(fb, &sh, fill_lo, fill_hi); } while (rr_dbf(&cnt));
                }
            }
        ...
        if (left < 0) {
            if (col >= 0) { rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src); }
        } else {
    ...
    for (;;) {
        ...
        if ((ctrl & RR_F_SPLIT_C) && !(ctrl & RR_F_WIDE) && (ctrl & RR_F_SKIP_D)) {
            ...
            if (col < 0) {                                     /* road near/off the left edge */
                int16_t e = rr_wadd((uint16_t)col, 8);
                if (e >= 0) { src += 8; rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src); }
            } else {
                dst += sx16((uint16_t)col);
                int16_t rem = rr_wsub((uint16_t)col, (uint16_t)stride);
                if (rem >= 0) {
                ...
                if (e < 0) {                                   /* 4 longs (masked 2nd on non-hi) */
                    rr_copy_long(fb, &dst, &src);
                    if (!(ctrl & RR_F_PLANE_HI)) rr_copy_long_masked(fb, &dst, &src, edge_mask);
                    else                         rr_copy_long(fb, &dst, &src);
                    rr_copy_long(fb, &dst, &src);
                    rr_copy_long(fb, &dst, &src);
                ...
                if (cells >= 0) {
                    Offset sh = r->dst; uint32_t cnt = (uint16_t)cells;
                    do { rr_fill_pair(fb, &sh, fill_lo, fill_hi); } while (rr_dbf(&cnt));
                }
            }
            r->dst += stride;
        ...
        if (col < 0) {                                         /* road off the left edge */
            if (rr_wadd((uint16_t)col, 8) >= 0) {
                src += 8; rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src);
            }
            r->dst += stride;
            if (!rr_dbf(&remaining)) break;
            continue;
        ...
        if (col_rem >= 0 && rr_wsub((uint16_t)col_rem, 8) >= 0) {
            ...
            if (n >= 0) {
                Offset shoulder = r->dst; uint32_t cnt = (uint16_t)n;
                do { rr_fill_pair_rev(fb, &shoulder, fill_lo, fill_hi); } while (rr_dbf(&cnt));
            }
            if (!rr_dbf(&remaining)) break;
            continue;
        }
        ...
        if (col_rem < 0) {
            col_rem = 0;
            rr_copy_long(fb, &dst, &src); rr_copy_long(fb, &dst, &src);
        } else {
            col_rem = rr_wsub((uint16_t)col_rem, 8);
        }
        ...
        for (;;) {
            width = rr_wsub((uint16_t)width, 8);
            if (width < 0) break;
            rr_fill_pair_rev(fb, &dst, fill_lo, fill_hi);
            if (!rr_dbf(&cnt)) break;
        }
        r->dst += stride;
        if (!rr_dbf(&remaining)) break;
...
static void rr_band_D(rr_regs *r, uint32_t rows_m1, int second) {
    ...
    for (;;) {
        ...
        int16_t edge_off = (int16_t)be16(r->edge);             /* used only by the no-split strip */
        r->edge += 2;                                          /* edge cursor advances every row */
        ...
        if (!skip_row) {
            int16_t col = (int16_t)((uint16_t)((int16_t)half_width >> 1) & RR_D7_WORD_MASK);
            Offset row_start = r->dst;
            Offset dst = row_start;

            if (col >= 0) {
                if (rr_wsub((uint16_t)col, (uint16_t)stride) >= 0) {
                    rr_fill_row(fb, row_start, fill_lo, fill_hi);        /* road fills the row */
                } else {
                    dst += sx16((uint16_t)col);
                    rr_draw_edge_cell(fb, &dst, &src, edge_mask);
                    ...
                    rr_fill_shoulder(fb, row_start, col, fill_lo, fill_hi);
                }
