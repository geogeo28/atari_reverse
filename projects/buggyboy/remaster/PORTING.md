...
mode-2/4/6 palette event — only its `obj_shade` output is consumed). **The demo boots AND restarts
through `rm_init_leg`** (`demo_main.c` `start_leg`: reset the owner structs, then derive the views via
`apply_player` + `ring_views_refresh` before the frame-0 draw), so the fixture no longer bakes any
per-leg leg-start scalar (see the fixture-shrink note below). Verified on the 68000: `run_demo.py`
...
**Fixture shrink (init_leg):** the demo used to bake a `*_INIT` snapshot of the oracle's `init_leg`
...
`rm_init_leg` produces the leg-start state natively at boot and on every restart. `gen_demo_fixture.py`
