 ...
 * console I/O) have NO image effect and return 0. Physbase/Logbase return OS_SCREEN_BASE;
 * Getrez returns 0 (low-res); Malloc bump-allocates from OS_HEAP_BASE; Mshrink/Mfree/Fclose/
 * Super return 0; Fopen returns OS_FILE_HANDLE.
 *
 * DEFERRED (serviced as return 0, effect NOT modeled): GEMDOS Fread (needs a file model),
 * XBIOS Supexec (needs nested execution), and GEM AES/VDI via trap #2. A function that
 * depends on these cannot be verified until the model is extended — see recreate/README.md.
 * OS_HEAP_BASE/OS_SCREEN_BASE are provisional low-memory arenas that only fit small blocks;
 * functions that Malloc large screen buffers need a larger IMAGE_SIZE first.
 */
#ifndef BB_OS_H
#define BB_OS_H
