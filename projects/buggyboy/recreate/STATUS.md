...
| `0x1110e` | `game_update` | 2452 | ✅ verified | per-frame in-race game-logic driver (root of intermission/init_playfield); no traps, returns each frame. Run-to-rts diff: common path (sections 1-11: input/fire/time/lean/steer-script/engine-speed/scroll/wheel/road-curve/edge-crash) 400-fuzz; course-advance (section 12: table shuffles, course-record unpack, sprite-modes 0/2/4/6, fx-block, checkpoint/collision/score) else+pull+crash+course fuzz; **event-jump-table dispatch idx 1-64 — every one of the 65 live `A_event_jumptable` entries**. The dispatcher (`gu_dispatch_event`) now reconstructs the full handler family in 0x11ba4..0x11f4c: flag_gate (1-11), score-message groups (13-21), checkpoint counters (22-24), the marker-decay object spawn (0x11d3a), the collision-object/spin/rpm-penalty spawns (0x11d64/0x11d8e/0x11dcc/0x11de2/0x11e16), the finish-line display (`gu_disp_finish`, 0x11e6a) and the bonus-number display (`gu_disp_bonus`, 0x11ebe → build_road_geometry + marker fx 8), plus the bare-rts no-ops. Verified two ways: `test_event_pending_dispatch` drives idx 1-64 through the section-6 path (whole-frame diff), and `test_event_handler_isolation` enters each distinct target at its own PC and runs to rts under all d6/d7 gates × collision_lock/speed/rpm/spin_state/road_curve/crash_phase preconditions. Residuals: none. The **collision-probe path** (`coll_mask` passed in D1, reloaded after `probe_collision`) is now diff-verified by `test_course_collision_probe`, which forces the flag bit on: `probe_collision` only word-writes D1, so D1.high stays = `coll_mask.high` — the exact bits the record-unpack select uses (`sel = (coll_mask >> 16) | rec_w0`), matching our unchanged `coll_mask`. The **idx 6/12 evt_flag_gate d7=6 variant** (`0x11c1a` `bra`s into the body past the type-match gate → advances the flag sequence unconditionally) was a real bug — the reconstruction ran the gated d7=7 path; now fixed via `g_evt_flag_gate_forced` and diff-verified by `test_evt_flag_gate_forced` (which fails against the old gated code). Sprite-mode 6 (the tunnel palette swap) pokes a hardware colour register at `0xffff824c+reg_sel` via `g_poke_color_reg` — an off-image write the oracle drops, so it is now **diff-verified** by `test_course_modes` (marker 0x8600); previously it wrongly wrote `recw-0x7db4` into the image (garbage palette on the tunnel). The checkpoint/collision/score-marker section's `play_event_tune` ids (checkpoint=5 @0x119c8, leg-end=1 @0x11a82, collision-marker=6 @0x11a92) and the entry marker fx (@0x1111a) are now **diff-verified** by directed tests (`test_gu_jingles.py` enters the sections-G/H/I tail @0x118b6 with event_type/score_str/ground_scan staged and the sound guard open so INITTUNE lands; `test_entry_marker_fx` fires the entry `handle_marker`). Reaching these fuzz-unreached branches **exposed four latent bugs** the image diff never hit: the `event_type` checkpoint/collision test read the word's high byte instead of the low byte the original's `cmpi.b` compares (checkpoint/collision events never fired); the `draw_checkpoint_anim` gate read `ground_scan_tbl+0` not the `+3` marker; the checkpoint char indexed `"SCORE/"` at `0x18000` not its real base `0x18540`; and the entry marker fx passed `event_type` to `handle_marker` where the original passes the `marker_pending` byte — all now fixed and covered. A later **on-hardware palette pass** (memory-snapshot diff of the recon vs the original at the leg-0 tunnel — `g_xbios_setpalette` is off-image so invisible to the whole-image oracle) caught four more: sprite-**mode 4** must `Setpalette` the RACE palette `0x17fa2`, not the `0x17fb0` scratch base — the original stages into `0x17fb0..` then `suba #$18,a0` rewinds A0 to `0x17fa2` before the trap, so loading the +7-shifted scratch (reg0=tan) corrupted the tunnel background; the section-G spin/fx select read `obj-0x1c/-0x1b` instead of `obj-2/-1`; `fx_block_2e` did a byte store where the original's last write is a `move.w` (word `0x003e`); and the section-H horizon-event dispatch passed `d5=0` instead of `horizon_row`/`horizon_row+2`, so handlers never cleared the `obj_active`/`event_type` slots and course/palette state drifted over real play. The latter three are green under `make test`; the mode-4 `Setpalette` target is off-image (verified by memory snapshot) |
...
| `0x12ec4` | `stop_music` | 50 | ✅ verified | guard fuzz (game_over); TURNOFF + clear fx/tune/vec + XBIOS Dosound(A0) — A0 = a `A_dosound_*` command list (countdown beeps / engine idle / crash), threaded as `list_off` and played via the off-image `g_dosound` seam (real XBIOS on-target). The A0 list itself is now **diff-verified** by the side-effect ledger (below): `test_main_racestart_dosound` enters main's race-start (@0x1021c) so the 0x10222 call site passes `A_dosound_beep`, and `test_dosound_ledger_sensitivity` proves a wrong A0 fails |
...
The image diff can't see two kinds of sound trigger: **off-image effects** (XBIOS `Dosound` writes the
YM2149, not RAM; `play_event_tune`/`handle_marker` only touch the image when their game_over/mzflag
guard is open) and **fuzz-unreached branches** (the call never runs). `make coverage-gap`
(`tools/coverage_gap.py`) flags every call site to a sound/OS sink (Dosound/INITTUNE/INITFX/play_event_
tune/handle_marker/stop_music) that no test executes, using the oracle's optional executed-PC coverage.

Both holes are now closed (`SOUND_VERIFY_HANDOFF.md`, improvements #2/#3):
- **#2 side-effect ledger** — the oracle's shim records each XBIOS `Dosound(A0)` list pointer in an
  ordered ledger (like the PSG tap), the candidate's `g_dosound` seam records its `list_off`, and
  `harness.differential` compares the two streams alongside the image — so a wrong/missing `Dosound`
  list fails a test even though it touches no RAM (`test_dosound_ledger_sensitivity` proves it).
- **#3 directed tests** — every formerly read-verified trigger is now reached by a test with its guard
  staged open so the effect lands in the diff: the three `game_update` jingles + entry marker fx, the
  two `update_highscore` tail jingles, and main's race-start `Dosound(A_dosound_beep)`.

`coverage_gap_allow.txt` is now **empty** and `make coverage-gap` reports **0 gaps**. Reaching the
checkpoint path for the first time also caught four latent `game_update` bugs (wrong `event_type` byte,
wrong `ground_scan` marker offset, wrong `"SCORE/"` base, wrong `handle_marker` arg) — see the
`game_update` row and the handoff. This is the payoff of the tooling: triggers the diff structurally
can't see are now either diff-verified or ledger-verified, not just read-verified.
