...
Zynaps' bss covers the TOS model's Malloc heap (`OS_HEAP_BASE = 0x20000` lies inside
`0x10000`–`0x6e96e`), so `project.toml` waives the kit's import-time overlap check by declaring that
the game issues no GEMDOS Malloc — justified there by an exhaustive byte scan of the text. If that
...
The kit therefore re-tests the claim after every oracle run instead of trusting the flag once. Until
this file existed the waiver was declared but never exercised here — every case in the suite runs a
pure leaf that traps not at all, so nothing ever drove the path the flag speaks about. Ported from
`projects/joust/recreate/test/test_heap_guard.py`, the other project the guard is armed for.
"""
import pytest
...
def malloc_stub(size):
    ...
    The `3f 3c 00 48` in the middle is exactly the byte pattern project.toml's scan reports as
    absent from ZYNAPS17.PRG at any alignment — which is what the waiver rests on.
    """
...
def test_guard_is_armed_for_this_project():
    ...
    assert kit_harness._HEAP_OVER_PROGRAM, (
        "OS_HEAP_BASE no longer lies inside Zynaps' program — the waiver in project.toml is now "
