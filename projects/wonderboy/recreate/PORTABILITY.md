...
| function | bytes | why |
|---|---:|---|
| `$a38 actor_behavior_type01_player` | 62 | slot 1 of the table — the player. **STILL NOT RECONSTRUCTED after batch 41 phase D, and this row's 62 is now checked rather than scanned**: the body was decoded from the raw image and is `$a38..$a75` = 62 bytes exactly, so unlike the two rows below it the scan did not lose phase here (there is no data inside this code). Its false-green edge is INHERITED and not its own: the routine has no transfer of any kind — nine `bsr`s, two memory guards and an `rts` — so every tier it carries comes from `$b1a` and `$151a`, whose `jmp $e5ba.l` unwinds are the path to the prompt. What blocks the port is not that edge but the X flag it must compose between `$a4a` and `$a4e` (STATUS.md, batch 41 phase D) |
