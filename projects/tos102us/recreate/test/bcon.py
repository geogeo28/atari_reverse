"""What BIOS Bconstat, Bconin and Bcostat share: one frame, one walk, one no-driver answer.

The three entries are the same four instructions over three different tables in RAM
(`src/bios/bcon.c`), so their batteries would otherwise keep three copies of how to enter them, how
...
# The three cores are one signature — (image, the D0 the trap dispatcher left, the device word), and
# the whole of D0 back — so it is declared once rather than three times over.
