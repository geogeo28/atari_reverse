...
Usage: mkprg.py tostest.elf tostest.bin TOSTEST.PRG

EVERY FIXUP IS CLASSIFIED BY WHERE IT LIVES AND THEN CHECKED AGAINST THE IMAGE. An entry that names
an offset which is not a pointer — section padding, or an entry left behind for a section a
collector discarded — relocates a live byte by adding the load base to it, which is corruption with
no symptom until some later trap lands on it. `abs_fixups` is where that is decided and says what
it refuses; `table_offsets` reads the emitted table back the way TOS will walk it. This directory's
links carry no `--gc-sections`, so the "past the end of the image" case cannot arise here and the
count in the final line is expected to read `+0`; the classification is kept because it costs
nothing and a mode that collected sections would otherwise meet the fault unguarded.

WHAT PROVES IT HERE is that the two .PRGs this script wraps run under the ORIGINAL ROM and leave a
ledger the host reads back (`tostest.py capture`, `tosbench.py capture`): a mis-relocated longword
is a program that never publishes its magic, and a run that never publishes is a driver that times
out rather than a picture nobody looks at.

COPIED FROM projects/wonderboy/recreate/atari/mkprg.py, which is itself a copy of Joust's and the
BuggyBoy build's. It is VERBATIM below this paragraph apart from this docstring and the memoisation
of `section_rows`/`nm_rows`; the tracked kit-candidate debt is the file itself, so an edit to the
behaviour belongs in all of the copies and a refactor of it belongs in `tools/recreate_kit/`
instead of here.
"""
import functools
import re
...
def refuse(reason):
    ...
    NOT an `assert`: this script runs under whatever `python3` the Makefile finds, and `-O` or
    ...
    downstream can see until the program is already running on the machine."""
...
# MEMOISED SO THE DOCSTRINGS BELOW ARE TRUE. Both say "THE ONE PARSE", and both had three callers
# between them — `bss_size`, `loaded_sections`, and `sym_value` twice — so a link ran `readelf -S`
# and `nm` twice each and the second reading of each was a reading of a file that could, in
# principle, have changed under it. One subprocess per tool per ELF, and the cache is keyed on the
# path this process was handed.
...
def nm_rows(elf):
    ...
    placed at an address, so it is dropped here rather than at each caller. `sym_value` is its only
    caller here, and asks for `_bss_start` and `_bss_end` off the same reading."""
