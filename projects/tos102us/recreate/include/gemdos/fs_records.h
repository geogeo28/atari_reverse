/* gemdos/fs_records.h — the GEMDOS file system's RECORD layer: making DNDs and OFDs, handing out a
 * handle for an open, and the text forms of a name and a path (`src/gemdos/fs_records.c`).
 *
 * The layouts are `include/gemdos/fs.h`'s; the byte copies and swaps under these are
 * `include/gemdos/fs_copy.h`'s. Each routine is declared with its ROM address and its arguments in
 * the ROM's own order.
 */
 ...
 * cleared — the `$fc5c9a` convention (`include/gemdos/fs.h`, "what the cores export"). */
