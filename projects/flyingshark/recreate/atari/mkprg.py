...
SHARED WITH projects/joust/recreate/atari/mkprg.py and projects/wonderboy/recreate/atari/mkprg.py —
the THREE files are identical BELOW THIS PARAGRAPH (each copy names the others, so the markers
differ and nothing else does), and a change to any belongs in all three. None is the canonical one:
`tools/recreate_kit/` is where this ought to live, and it is registered as a kit candidate in
projects/joust/recreate/atari/README.md ("Reviewed and deferred"), in projects/wonderboy/recreate/
STATUS.md's batch 43 phase A queue, and — now that there is a THIRD copy — in ../STATUS.md's
"Follow-ups the kit should absorb". Until that move happens, this marker is the only thing
linking them.
"""
import re
import struct
